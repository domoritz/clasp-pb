...
public:
	...
	struct Options {
		...
		uint32  maxFrozen;/**< run only if percent of frozen vars <= maxFrozen      */
		uint32  bce;      /**< blocked clause elimination (0=off, 1=limited, 2=full)*/
		uint32  maxClause;/**< run only if #clauses < maxClause                     */
		uint32  elimPure; /**< eliminate pure literals?                             */
	} options;
	enum { event_type = 0xD39765C2 };
	struct Event : public PreprocessEvent {
		Event(SatElite* p, char t, uint32 i, uint32 m) : PreprocessEvent(event_type), self(p), op(t), cur(i), max(m) {}
		SatElite* self;
		char      op;
		uint32    cur;
		uint32    max;
